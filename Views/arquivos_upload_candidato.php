...
// O arquivo passou em todas as verificações, hora de tentar movê-lo para a pasta
    ...
    if (move_uploaded_file($_FILES['arquivo']['tmp_name'], $_UP['pasta'] . $novoNome)) {
        ...
        //******//
        //APAGAR O ARQUIVO ANTERIOR DO SERVIDOR
        // apagaArquivoAnterior($_POST['tipo_do_documento'], $_POST['dados_pasta']);
        //******//
        ...
        echo '&nbsp<a href="edicao_documentos_pasta_promo_candidato.php?id_da_pasta=' . $_POST['dados_pasta'] . '"><button class="btn btn-outline-success active" type="button">Voltar</button></a>';
    } else {
...
function salvaNoBanco($documento, $caminho, $id)
{
    ...
    $stmt = $conn->query("SELECT * FROM documento WHERE descricao = '" . $documento . "' AND pasta_promo_id = '" . $id . "'");
    $res = $stmt->fetch(PDO::FETCH_ASSOC);
    
    if (!$res) {
        $stmt = $conn->prepare('INSERT INTO documento (descricao, caminho_do_arquivo, pasta_promo_id) VALUES (:descricao, :caminho, :pasta)');
        $stmt->execute(array(
            ':descricao' => $documento,
            ':caminho' => $caminho,
            ':pasta' => $id
        ...
        $apagar = $res['caminho_do_arquivo'];
        unlink($apagar);

        $stmt = $conn->prepare('UPDATE documento SET descricao = :descricao, caminho_do_arquivo = :caminho WHERE pasta_promo_id = :pasta');
        $stmt->execute(array(
            ':descricao' => $documento,
            ':caminho' => $caminho,
            ':pasta' => $id
